...
function race9() {
    return $.ajax({
		...
		data.pastraces[0].results.forEach(function(k)
		{
			...
			Race9Score[k.player] = { name: k.player, value: pointsDistribution[place.toString()] };
		});
	});
}
...
function race11() {
    return $.ajax({
		url: '/scoresboard/Season2/Race11.json',
		method: 'GET'
	}).then(function(data) {
		data.pastraces[0].results.forEach(function(k)
		{
...
{	
	...
	Object.keys(Race11Score).forEach(function(key) 
	{
    	if(Leaderboard[key])
		{
			Leaderboard[key].value = Leaderboard[key].value + Race11Score[key].value;
			Leaderboard[key].nraces = Leaderboard[key].nraces + 1;
		}
		else
		{
			Leaderboard[key] = Race11Score[key];
			Leaderboard[key].nraces = 	1;
		}
	});
